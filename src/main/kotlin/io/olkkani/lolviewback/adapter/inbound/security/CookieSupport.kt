...
object CookieSupport {
    ...
    fun buildAccessTokenCookie(
        value: String,
        maxAgeSeconds: Long,
    ): ResponseCookie = cookie(ACCESS_TOKEN_COOKIE_NAME, value, maxAgeSeconds, path = "/")

    fun buildRefreshTokenCookie(
        value: String,
        maxAgeSeconds: Long,
    ): ResponseCookie = cookie(REFRESH_TOKEN_COOKIE_NAME, value, maxAgeSeconds, path = "/auth")
